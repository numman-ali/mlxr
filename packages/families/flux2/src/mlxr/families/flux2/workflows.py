...
class Flux2WorkflowStrategy(FamilyWorkflowStrategy):
    family_id = "flux2"
        ...
        return WorkflowPlan(
            ...
            stages=[
                WorkflowStageSpec(
                    ...
                    summary="Run the selected FLUX.2 generation or editing task through the shared runtime job API.",
                    task=task,
    ...
    def to_job_request(
        ...
        flux_extensions = dict(family_extensions(extensions.get("flux2")))
        if flux_extensions:
            extensions["flux2"] = flux_extensions
        extensions["workflow"] = {
...
def _validate_references(references: list[WorkflowReference]) -> None:
    for reference in references:
        if reference.kind not in {"image", "lora"}:
            raise ValueError(
                "The current FLUX.2 workflow strategy only supports image and lora references"
            )
