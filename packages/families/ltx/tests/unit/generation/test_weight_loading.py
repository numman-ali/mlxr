...
class WeightLoadingTests(unittest.TestCase):
    def test_align_module_dtype_to_weights_uses_dominant_dtype(self) -> None:
        ...
        self.assertEqual(target_dtype, mx.bfloat16)
        self.assertEqual(module.weight.dtype, mx.bfloat16)

    def test_transformer_loader_uses_checkpoint_weight_dtype(self) -> None:
        ...
        self.assertIsInstance(loaded_parameters, dict)

        dtypes = {value.dtype for value in loaded_parameters.values()}
        self.assertEqual(dtypes, {mx.bfloat16})

    def test_transformer_model_registers_block_parameters(self) -> None:
        config = LTXModelConfig.from_dict(
            {
        ...
        self.assertIn("transformer_blocks.0.attn1.to_q.weight", parameter_tree)
        self.assertIn("transformer_blocks.1.ff.proj_in.weight", parameter_tree)
        self.assertIn("transformer_blocks.0.scale_shift_table", parameter_tree)
        self.assertGreater(len(parameter_tree), 30)
