...
- the current text-first prompt stack now uses a shared repo-owned shaper for explicit audio details plus `natural_audio` / `no_music` / duration / orientation cues, and the first family-local negative-guidance plus rescaled-CFG slice now lets the 10-second dog showcase clear the stronger Gemini video-plus-audio review bar with barking and natural outdoor ambience
...
- the natural-audio showcase story is still separate: the improved 10-second dog scene is the first scene-level success, but nature and natural-vintage realism still need to clear the same video-plus-audio Gemini review bar on the newer guidance path
