...
- the current text-first prompt stack now uses a shared repo-owned shaper for explicit audio details plus `natural_audio` / `no_music` / duration / orientation cues, the showcase runner adds scenario-level `audio_intent` plus style-aware anti-music negatives for `naturalistic`, `documentary`, and `vintage` scenes, and the real workflow path can now read the same `style_family` hint through `intent.extensions["ltx"]` without widening the shared workflow surface
