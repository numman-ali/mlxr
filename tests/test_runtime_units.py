...
class RuntimeUnitTests(unittest.TestCase):
    ...
    def test_ltx_generation_backend_reads_nested_bwe_vocoder_contract(self) -> None:
        ...
        self.assertEqual(config.resblock, "AMP1")
        self.assertEqual(config.activation, "snakebeta")
        self.assertFalse(config.use_tanh_at_final)
        self.assertTrue(config.apply_final_activation)
        self.assertFalse(config.use_bias_at_final)
