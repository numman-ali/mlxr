...
| image generation (`Z-Image`, `Qwen-Image`, `FLUX.2`) | image generation and editing | Hugging Face first, later ModelScope pressure and possible API-backed host pressure | `image_diffusion` | size constraints, negative prompting, multi-image editing, prompt rewrite or upsampling sidecars, bilingual prompt fidelity | `png`, `jpg`, optional intermediate latents | high | attention helpers, VAE decode, tiled decode | high because official `ltx-desktop` already exposes `Z Image Turbo` and product workflows want reusable keyframes | Phase C |
