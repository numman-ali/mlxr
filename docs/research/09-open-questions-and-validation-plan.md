...
- the current AV bridge now also decodes checkpoint-backed audio and exports it as muxed runtime-managed `mp4` or standalone `wav`, and the repo-owned MLX `AMP1` base-vocoder bridge has restored audible real-weight output at the safe and visual-gate rungs
...
- this moves the next blocker forward again: the remaining truth gap is no longer prompt encoding, preview generation, silent bridge drift, first subject formation, or silent audio export, but broader capability completion beyond the distilled two-stage baseline, full `LTX-2.3` BWE audio parity, followed by recommended-resolution and HQ-profile validation on the real path
