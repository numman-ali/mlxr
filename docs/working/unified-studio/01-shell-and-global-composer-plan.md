...
- expanded: centered floating drawer
- collapsed: compact trailing command pill
- hidden: user-dismissed resting state, also forced while blocking overlays or the library viewer are open
...
- prompt field
- seeded-context chips for the focused asset and active references
...
- a separate advanced-settings popover for this tranche

## Canonical Mode Model

The shell owns a single global draft, backed by `CreationDraft`, with this interpretation:
...
- The explicit `New Project` action in the top-level `Library` browser creates and opens a blank project immediately.
- Submitting from the top-level `Library` browser without opening a project first creates a new project after runtime acceptance.
...
- Scrollable surfaces should read the shell-owned courtesy inset rather than mirroring live composer height like a footer.
...
3. If project history exists, show `Library` focused on the active or most recent project continuity path rather than restoring `Models` or `Settings`.

The shell no longer includes a hidden Studio fallback route.
...
Startup routing still prefers the current product heuristic over blindly restoring the last destination:

1. `Models` when starter-model setup is still pending
2. `Library` when meaningful project history exists
3. `Home` otherwise
...
- The old Studio destination is removed from the app shell.
